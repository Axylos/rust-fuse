...
use fuse_sys::abi::{fuse_open_out, fuse_write_out, fuse_statfs_out, fuse_lk_out, fuse_bmap_out, fuse_ioctl_out};
...
impl ReplyIoctl {
    ...
    pub fn ioctl(mut self, result: i32, data: &[u8]) {
        let header = fuse_ioctl_out {
            ...
            flags: 0,
            in_iovs: 1,
            out_iovs: if data.len() > 0 { 1 } else { 0 },
        };
        ...
        debug!("IOCTL() header_bytes {}, data {}", header_bytes.len(), data.len());
